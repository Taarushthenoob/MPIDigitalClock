...
#BP=0000h#
	...
	dw 0000h
	dw 0000h
	dw 76 dup(0000h);set all interrupts before int 40h as zero  
	  
	dw opt1; set int 40h to opt1
	dw 0000h
	dw start1;set int 41h  to start1
	dw 0000h
	db 380 dup (0); set rest of IVT as zero, since it is not used
	db 380 dup(0)

st1:    sti 					; clear interrupt flag, because we won't be using maskable innterrupts
	...
	; 8254 is running on 2.5MHz clock => We must divide by (25000)d or (61A8)h in order to get a out frequency of 100Hz which is the clock input for counter1
...
polling: 
		...
		jne x11				;Keep polling
		call buzzer			;alarm is ON
x11:	jmp polling			;Keep polling		

        ;end of main program
        
        
        ; ISR associated with INTR from IR0 (given by 8259) compares position of thumbswitch and decides which function to call based on it 
	    ; if witch is set to set_hour => this ISR will call the functionn set_hour		
				
opt1:   
        sti
		
    	in al,port1c        ;debounce in slideswitch
		mov bl,al
		call delay_20ms
		in al,port1c
		cmp al,bl				; if the two values are diff, iret
		jne mn8			
...
mn8:		
        mov       al,20h
	    out       port3a,al
        iret            ; After one of them has been done, we need to repeat the polling process

; end of ISR associated with INTR from IR0. (given by 8259(int 40h))

	
	; ISR associated with INTR from IR1 (given by 8259) compares seconds, minutes, etc increments them if necessary 
...
; end of ISR associated with INTR from IR1. (given by 8259(int 41h))
